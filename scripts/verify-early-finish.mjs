/**
 * Regression: finish messages during the 3-2-1-GO countdown must not crown a winner.
 * Usage: node scripts/verify-early-finish.mjs
 */
...
async function main() {
  ...
  const room = `early-finish-${Date.now().toString(36)}`;
  ...
  if (earlyA || earlyB) {
    throw new Error("finish during countdown incorrectly crowned a winner");
  }
  console.log("ok  finish rejected during countdown");
  ...
  if (!a || !b || a.winnerId !== welcome.id || b.winnerId !== welcome.id) {
    throw new Error(`finish after countdown failed: ${JSON.stringify({ a, b })}`);
  }
  console.log("ok  finish accepted after countdown");
